...
async function syncItemToFirestore(collectionName, itemData, action) {
  if (window.db && window.firestore) {
    try {
      ...
      alert(`حصل خطأ في حفظ ${collectionName} في فايربيس (قد تكون قواعد البيانات Rules تمنع الكتابة): ${e.message}`);
    }
  } else {
    alert("لم يتم تجهيز فايربيس بعد. الرجاء الانتظار بضع ثوان والمحاولة مرة أخرى.");
  }
}
